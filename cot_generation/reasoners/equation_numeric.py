"""
数值方程推理器 - 处理 equation_numeric 类题目
=============================================

移植自原始方案的核心算法：
- 解析 "数字+运算符+数字 = 结果" 格式的示例
- 尝试常见运算（加减乘除、拼接、绝对差等）
- 支持操作数反转和结果反转
- 支持前缀/后缀格式检测
- 支持稀有运算（逐位运算、行列式等）

确定性保证：穷举所有候选运算，找到能解释所有示例的那个
"""
...
def _apply_op(found: FoundOp, a_str: str, b_str: str) -> tuple[str, list[str]]:
    ...
    expr_s = _expr(found.op_name, ta, tb)
    inter = _expr_intermediate(found.op_name, ta, tb)
    if expr_s and inter:
        detail = f" {expr_s} = {inter} ="
    elif expr_s:
        detail = f" {expr_s} ="
    else:
        detail = ""
...
def _reasoning_equation_numeric_core(
    parsed: list[tuple[str, str, str, str]], question: str
) -> Optional[str]:
    """
    核心 equation_numeric 推理算法（忠实移植原始方案）。
    
    parsed: [(a_str, op_char, b_str, output_str), ...]
    question: 查询表达式字符串
    """
    lines: list[str] = []
            ...
            for rev_ops, rev_res in ((True, True), (False, False), (True, False), (False, True)):
                ...
                for cand_name, cand_res in candidates:
                    ...
                    for i, (ax, bx, exp_x) in enumerate(rotated):
                        ...
                        raw = next(
                            r for n, r in _all_candidates(int(rax), int(rbx), rax, rbx)
                            if n == cand_name
                        )
                        ...
                        if fin != exp_x:
                            all_pass = False
                        val_display = f"{raw} -rev-> {fin}" if rev_res else fin
                        if i > 0:
                            parts.append(f"f({rax},{rbx}) ->{detail_x} {val_display} {status}")
                        else:
                            parts.append(f"f({rax}, {rbx}) ={detail_x} {val_display} {status}")
                        if fin != exp_x:
                            break
                    ...
                    if not found:
                        found = FoundOp(
                            op_name=cand_name, rev_ops=rev_ops, rev_res=rev_res,
                            fmt=detected_fmt, op_char=op_char,
                        )
    ...
    if effective_q_op != q_op:
        ...
        abs_diff_op = FoundOp(
            op_name="absolute difference", rev_ops=False, rev_res=False,
            fmt=found_ops[effective_q_op].fmt, op_char=q_op or "",
        )
    ...
    def solve(self, problem: Problem) -> ReasoningResult:
        ...
        try:
            ...
            for expr, result in examples:
                ...
                parsed.append((a, op, b, result.strip()))
            ...
            if not parsed:
                return ReasoningResult(
                    ...
                    category="cryptarithm",
                    success=False,
                    error_message="无法解析为数值方程格式"
                )
            
            reasoning_text = _reasoning_equation_numeric_core(parsed, query_expr.strip())
